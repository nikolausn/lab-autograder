...
if [ "$PARTID" == "$EMAIL_PART_ID" ]; then
  GRADER_DIRECTORY=01_email
  SUBMISSION_FILE=email.txt
elif [ "$PARTID" == "$PHONE_PART_ID" ]; then
  GRADER_DIRECTORY=02_phone
  SUBMISSION_CLASS=phone.txt
else
...
#ls -ltr /shared/submission
#echo "subm: $SUBMISSION_CLASS"
